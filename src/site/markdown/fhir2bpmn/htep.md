 ...
 * There is not yet a support for nested gateway in the transformation. This leads to a partially invalid result.
 * Due to the fact, that our decision logic for xor-gateways is tree-based, meaning that all action must be nested actions of the gateway. This does not allow "jumps" to actions that are in other gateways.
