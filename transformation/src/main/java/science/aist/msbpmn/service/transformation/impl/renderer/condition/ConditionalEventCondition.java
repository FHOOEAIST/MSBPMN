...
/**
 * <p>Condition for ConditionalEvents</p>
 *
 * @author Andreas Pointner
 */
@Component
public class ConditionalEventCondition implements RendererCondition<Vertex<PlanDefinition.PlanDefinitionActionComponent, Void>> {

    private final Predicate<Vertex<PlanDefinition.PlanDefinitionActionComponent, Void>> condition =
            element -> element.tryGetMetaTagValue(TransformationConstants.EVENT_META_TAG).map(x -> x == BpmnEventTypes.EVENT_CONDITION).orElse(false);
